  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-soft p-6">
        ...
        {/* Section Description */}
        {activeSection && (
          <div className="mt-4 p-4 bg-light-beige-50 rounded-lg">
...
function UserOverview({ profile, listings }: { profile: Profile; listings: Listing[] }) {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-vibrant-orange-50 to-warm-yellow-50 rounded-2xl p-8">
        <div className="flex items-center gap-6">
          <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-medium">
            {profile.avatar_url ? (
              ...
              <User className="w-10 h-10 text-vibrant-orange-600" />
            )}
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl p-6 shadow-soft text-center">
          <div className="w-12 h-12 bg-vibrant-orange-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <Package className="w-6 h-6 text-vibrant-orange-600" />
          </div>
          <div className="text-2xl font-bold text-dark-green-600 mb-1">{activeListings.length}</div>
          <div className="text-sm text-dark-green-500">Active Listings</div>
        </div>

        <div className="bg-white rounded-xl p-6 shadow-soft text-center">
          <div className="w-12 h-12 bg-warm-yellow-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <BarChart3 className="w-6 h-6 text-warm-yellow-600" />
          </div>
          <div className="text-2xl font-bold text-dark-green-600 mb-1">{totalViews}</div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-soft text-center">
          <div className="w-12 h-12 bg-dark-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <Heart className="w-6 h-6 text-dark-green-600" />
          </div>
          <div className="text-2xl font-bold text-dark-green-600 mb-1">{totalFavorites}</div>
          <div className="text-sm text-dark-green-500">Favorites</div>
        </div>

        <div className="bg-white rounded-xl p-6 shadow-soft text-center">
          <div className="w-12 h-12 bg-vibrant-orange-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <MessageCircle className="w-6 h-6 text-vibrant-orange-600" />
          </div>
          <div className="text-2xl font-bold text-dark-green-600 mb-1">{profile.trust_score}</div>
          <div className="text-sm text-dark-green-500">Trust Score</div>
        </div>
      </div>

      {/* Recent Activity */}
      <div className="bg-white rounded-2xl shadow-soft p-6">
        <h3 className="text-lg font-semibold text-dark-green-600 mb-4">Recent Activity</h3>
        {activeListings.length > 0 ? (
            ...
            {activeListings.slice(0, 3).map((listing) => (
              <div key={listing.id} className="flex items-center gap-4 p-3 bg-light-beige-50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-dark-green-500">€{listing.price} • {listing.views} views</p>
                </div>
          ...
          <div className="text-center py-8 text-dark-green-500">
            <Package className="w-12 h-12 mx-auto mb-3 opacity-50" />
            <p>No listings yet. Start by creating your first game listing!</p>
          </div>
        )}
      </div>
    </div>
